namespace ART.Domotica.Worker.Consumers
{
    ...
    public class DeviceSensorsConsumer : ConsumerBase, IDeviceSensorsConsumer
    {
        ...
        {
            BasicQueueDeclare(DeviceSensorsConstants.SetReadIntervalInMilliSecondsQueueName);
            ...
            _model.BasicConsume(DeviceSensorsConstants.SetReadIntervalInMilliSecondsQueueName, false, _setReadIntervalInMilliSecondsConsumer);
        ...
        public async Task SetPublishIntervalInMilliSecondsReceivedAsync(object sender, BasicDeliverEventArgs e)
        {
            ...
            var data = await deviceSensorsDomain.SetPublishIntervalInMilliSeconds(message.Contract.DeviceTypeId, message.Contract.DeviceDatasheetId, message.Contract.DeviceId, message.Contract.IntervalInMilliSeconds);
        ...
        public async Task SetReadIntervalInMilliSecondsReceivedAsync(object sender, BasicDeliverEventArgs e)
        {
            ...
            var message = SerializationHelpers.DeserializeJsonBufferToType<AuthenticatedMessageContract<DeviceSetIntervalInMilliSecondsRequestContract>>(e.Body);
            var deviceSensorsDomain = _componentContext.Resolve<IDeviceSensorsDomain>();
            var data = await deviceSensorsDomain.SetReadIntervalInMilliSeconds(message.Contract.DeviceTypeId, message.Contract.DeviceDatasheetId, message.Contract.DeviceId, message.Contract.IntervalInMilliSeconds);

            var applicationMQDomain = _componentContext.Resolve<IApplicationMQDomain>();
            var applicationMQ = await applicationMQDomain.GetByApplicationUserId(message);

            //Enviando para View
            var viewModel = Mapper.Map<DeviceSensors, DeviceSensorsSetReadIntervalInMilliSecondsModel>(data);
            var viewBuffer = SerializationHelpers.SerializeToJsonBufferAsync(viewModel, true);
            var rountingKey = GetInApplicationRoutingKeyForAllView(applicationMQ.Topic, DeviceSensorsConstants.SetReadIntervalInMilliSecondsViewCompletedQueueName);
            _model.BasicPublish(defaultExchangeTopic, rountingKey, null, viewBuffer);
