...
namespace ART.Domotica.Worker.Consumers
{
    public class DSFamilyTempSensorConsumer : ConsumerBase, IDSFamilyTempSensorConsumer
    {
        ...
        private async Task GetAllByDeviceInApplicationIdReceivedAsync(object sender, BasicDeliverEventArgs e)
        {
            ...
            var routingKey = GetApplicationRoutingKeyForIoT(applicationMQ.Topic, deviceBrokerSetting.Topic, DSFamilyTempSensorConstants.GetAllByDeviceInApplicationIdCompletedIoTQueueName);
        ...
        public async Task GetAllResolutionsReceivedAsync(object sender, BasicDeliverEventArgs e)
        {
            ...
            var rountingKey = GetApplicationRoutingKeyForView(message.SouceMQSession, DSFamilyTempSensorConstants.GetAllResolutionsViewCompletedQueueName);
        ...
        public async Task SetResolutionReceivedAsync(object sender, BasicDeliverEventArgs e)
        {
            ...
            var viewBuffer = SerializationHelpers.SerializeToJsonBufferAsync(viewModel);            
            var rountingKey = GetApplicationRoutingKeyForView(message.SouceMQSession, DSFamilyTempSensorConstants.SetResolutionViewCompletedQueueName);
            _model.BasicPublish(exchange, rountingKey, null, viewBuffer);

            var espDeviceDomain = _componentContext.Resolve<IESPDeviceDomain>();
            var applicationMQ = await espDeviceDomain.GetApplicationMQ(viewModel.DeviceId);
            var deviceBrokerSetting = await espDeviceDomain.GetDeviceBrokerSetting(viewModel.DeviceId);
            ...
            var deviceBuffer = SerializationHelpers.SerializeToJsonBufferAsync(deviceMessage);
            var routingKey = GetApplicationRoutingKeyForIoT(applicationMQ.Topic, deviceBrokerSetting.Topic, DSFamilyTempSensorConstants.SetResolutionIoTQueueName);
            _model.BasicPublish(exchange, routingKey, null, deviceBuffer);
        ...
        private async Task SetScaleReceivedAsync(object sender, BasicDeliverEventArgs e)
        {
            ...
            var viewBuffer = SerializationHelpers.SerializeToJsonBufferAsync(viewModel);            
            var rountingKey = GetApplicationRoutingKeyForView(message.SouceMQSession, DSFamilyTempSensorConstants.SetScaleViewCompletedQueueName);
            _model.BasicPublish(exchange, rountingKey, null, viewBuffer);
            ...
            var deviceBuffer = SerializationHelpers.SerializeToJsonBufferAsync(deviceMessage);
            var routingKey = GetApplicationRoutingKeyForIoT(applicationMQ.Topic, deviceBrokerSetting.Topic, DSFamilyTempSensorConstants.SetScaleIoTQueueName);
            _model.BasicPublish(exchange, routingKey, null, deviceBuffer);
        ...
        public async Task SetLabelReceivedAsync(object sender, BasicDeliverEventArgs e)
        {
            ...
            var rountingKey = GetApplicationRoutingKeyForView(message.SouceMQSession, DSFamilyTempSensorConstants.SetLabelViewCompletedQueueName);
            _model.BasicPublish(exchange, rountingKey, null, viewBuffer);
        ...
        public async Task SetAlarmOnReceivedAsync(object sender, BasicDeliverEventArgs e)
        {
            ...
            viewModel.DeviceId = data.SensorsInDevice.Single().DeviceBaseId;
            var viewBuffer = SerializationHelpers.SerializeToJsonBufferAsync(viewModel, true);            
            var rountingKey = GetApplicationRoutingKeyForView(message.SouceMQSession, DSFamilyTempSensorConstants.SetAlarmOnViewCompletedQueueName);
            _model.BasicPublish(exchange, rountingKey, null, viewBuffer);
            ...
            var deviceBuffer = SerializationHelpers.SerializeToJsonBufferAsync(deviceMessage);
            var routingKey = GetApplicationRoutingKeyForIoT(applicationMQ.Topic, deviceBrokerSetting.Topic, DSFamilyTempSensorConstants.SetAlarmOnIoTQueueName);
            _model.BasicPublish(exchange, routingKey, null, deviceBuffer);
        ...
        public async Task SetAlarmCelsiusReceivedAsync(object sender, BasicDeliverEventArgs e)
        {
            ...
            viewModel.DeviceId = data.SensorsInDevice.Single().DeviceBaseId;
            var viewBuffer = SerializationHelpers.SerializeToJsonBufferAsync(viewModel, true);            
            var rountingKey = GetApplicationRoutingKeyForView(message.SouceMQSession, DSFamilyTempSensorConstants.SetAlarmCelsiusViewCompletedQueueName);
            _model.BasicPublish(exchange, rountingKey, null, viewBuffer);
            ...
            var deviceBuffer = SerializationHelpers.SerializeToJsonBufferAsync(deviceMessage);
            var routingKey = GetApplicationRoutingKeyForIoT(applicationMQ.Topic, deviceBrokerSetting.Topic, DSFamilyTempSensorConstants.SetAlarmCelsiusIoTQueueName);
            _model.BasicPublish(exchange, routingKey, null, deviceBuffer);
        ...
        public async Task SetAlarmBuzzerOnReceivedAsync(object sender, BasicDeliverEventArgs e)
        {
            ...
            viewModel.DeviceId = data.SensorsInDevice.Single().DeviceBaseId;
            var viewBuffer = SerializationHelpers.SerializeToJsonBufferAsync(viewModel, true);            
            var rountingKey = GetApplicationRoutingKeyForView(message.SouceMQSession, DSFamilyTempSensorConstants.SetAlarmBuzzerOnViewCompletedQueueName);
            _model.BasicPublish(exchange, rountingKey, null, viewBuffer);
            ...
            var deviceBuffer = SerializationHelpers.SerializeToJsonBufferAsync(deviceMessage);
            var routingKey = GetApplicationRoutingKeyForIoT(applicationMQ.Topic, deviceBrokerSetting.Topic, DSFamilyTempSensorConstants.SetAlarmBuzzerOnIoTQueueName);
            _model.BasicPublish(exchange, routingKey, null, deviceBuffer);
        ...
        public async Task SetChartLimiterCelsiusReceivedAsync(object sender, BasicDeliverEventArgs e)
        {
            ...
            viewModel.DeviceId = data.SensorsInDevice.Single().DeviceBaseId;
            var viewBuffer = SerializationHelpers.SerializeToJsonBufferAsync(viewModel, true);            
            var rountingKey = GetApplicationRoutingKeyForView(message.SouceMQSession, DSFamilyTempSensorConstants.SetChartLimiterCelsiusViewCompletedQueueName);
            _model.BasicPublish(exchange, rountingKey, null, viewBuffer);
            ...
            var deviceBuffer = SerializationHelpers.SerializeToJsonBufferAsync(deviceMessage);            
            var routingKey = GetApplicationRoutingKeyForIoT(applicationMQ.Topic, deviceBrokerSetting.Topic, DSFamilyTempSensorConstants.SetChartLimiterCelsiusIoTQueueName);
            _model.BasicPublish(exchange, routingKey, null, deviceBuffer);
