var BancoIndexScript = function () {
    ...
    var handleInit = function () {
        ...
        var tableColumnToggler = $('#banco_column_toggler');
    ...
    var handleInitDataTable = function () {
        ...
        oTable = $('#dataTable').dataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url": ApplicationScript.getAppWebApiUrl() + "/api/banco/masterViewList?ativo=" + getAtivo(),
           ...
           {               
               ...
               "render": function (data, type, row) {
                    ...
                    if (row.logoStorageObject) {
                        srcLogoBanco = "/api/banco/logo/" + row.logoStorageObject;
                    }
                    else {
                        srcLogoBanco = "../assets/admin/img/bank.svg";
                    }

                    return "<img style='width:16px;height:11px' class='flag' src='" + srcLogoBanco + "'/>";
               },               
           },
           {
               "targets": 1,
           ...
           },
           {
               ...
               "render": function (data, type, row) {
                    ...
                    actionControls += "<a href=\'/Banco/Detail/" + row.bancoId + "\' class=\'btn default btn-xs grey\'><i class=\'fa fa-edit\'></i> Editar</a>";
                    actionControls += "<a onclick='BancoIndexScript.removeBanco(\"" + row.bancoId + "\",\"" + row.nomeAbreviado + "\");' href='#' class='btn default btn-xs grey'><i class='fa fa-trash-o'></i> Excluir</a>";
                    actionControls += "</div>";
    ...
    var handleRemoveBanco = function (id, nome) {

        var message = "Deseja realmente excluir o banco '" + nome + "'?";

        bootbox.confirm(message, function (result) {
            if (result) {
                $.ajax({
                    type: "DELETE",
                    url: ApplicationScript.getAppWebApiUrl() + '/api/banco/' + id,
                    headers: { 'Authorization': 'bearer ' + ApplicationScript.getToken() },
                    content: "application/json; charset=utf-8",
                    dataType: "json",
                    async: true,
                    ...
                    if (data) {
                        ...
                        bootbox.alert({ message: "O banco '" + nome + "' n&atilde;o pode ser exclu&iacute;do pois j&aacute; est&aacute; sendo utilizado pelo sistema." });
                    }
    ...
    var refreshDataTable = function () {
        ...
        oSettings.ajax.url = '/api/banco/masterViewList?ativo=' + getAtivo();
    ...
    return {
        ...
        removeBanco: function (id, nome) {
            handleRemoveBanco(id, nome);
        }
    };
}();