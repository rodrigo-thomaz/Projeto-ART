var LancamentoPagoRecebidoIndexScript = function () {
    ...
    var handleInit = function () {
        ...
        var title = 'Lan\u00e7amentos pagos e recebidos';
    ...
    var loadPeriodos = function () {        
        ...
        $.ajax({
            type: "GET",
            url: ApplicationScript.getAppWebApiUrl() + '/api/lancamentoPagoRecebido/periodos/',
    ...
    var handleInitTableLancamento = function () {
                    ...
                    },
                    {
                        ...
                        "render": function (data, type, row) {
                            ...
                            if (row.tipoProgramacao == 'T') {
                                ...
                                actionControls += "<a href=\'/LancamentoPagoRecebido/Detail/" + row.lancamentoId + "/" + row.tipoTransacao + "' class=\'btn default btn-xs grey\'><i class=\'fa fa-edit\'></i> Editar</a>";
                                actionControls += "<a onclick='LancamentoPagoRecebidoIndexScript.removeLancamento(\"" + row.lancamentoId + "\",\"" + row.tipoTransacao + "\",\"" + row.historico + "\");' href='#' class='btn default btn-xs grey'><i class='fa fa-trash-o'></i> Excluir</a>";
                            }
        ...
        */
        ...
        function fnFormatDetails(oTable, nTr) {            
            ...
            var sOut = '';
            var tableName = 'tableConciliacao' + aData.lancamentoId;

            sOut += '<table class="table table-striped table-bordered table-hover table-full-width" id="' + tableName + '">';
            sOut += '<thead>';
            sOut += '<tr>';
            sOut += '<th>Conciliado</th>';
            sOut += '<th>Data</th>';
            sOut += '<th>Conta</th>';
            sOut += '<th>Hist&oacute;rico</th>';
            sOut += '<th style="text-align: right; width:130px;">Valor movimento</th>';
            ...
            var movimentos = ConciliacaoSharedScript.getMovimentos(aData.lancamentoId, aData.tipoTransacao);

            $.each(movimentos, function (index, value) {

                var conciliado = value.estaConciliado ? 'Sim' : 'N&atilde;o';

                sOut += '<tr>';
                sOut += '<td>' + conciliado + '</td>';
                sOut += '<td>' + value.dataMovimento + '</td>';
                sOut += '<td>' + value.contaNome + '</td>';
                sOut += '<td>' + value.historico + '</td>';
                sOut += '<td style="text-align: right; width:130px;">' + value.valorMovimento + '</td>';
                sOut += '<td style="text-align: right; width:130px;">' + value.valorConciliado + '</td>';
                sOut += '<td><a href="/Movimento/Detail/' + value.movimentoId + '/' + value.tipoTransacao + '" class="btn default btn-xs grey"><i class="fa fa-edit"></i> Detalhes</a></th>';
                sOut += '</tr>';
            });
    ...
    var handleRemoveLancamento = function (lancamentoId, tipoTransacao, historico) {
        ...
        var message = "Deseja realmente excluir o pagamento e as concilia&ccedil;&otilde;es do Lan\u00e7amento \"" + historico + "\"?  \n  \n";
        message += "Observa&ccedil;&atilde;o: O Lan\u00e7amento retornara para o Contas a Pagar/Receber e as movimenta&ccedil;&otilde;es deixar&atilde;o de estar conciliadas.";

        bootbox.confirm(message, function (result) {
            if (result) {
                $.ajax({
                    type: "DELETE",
                    url: ApplicationScript.getAppWebApiUrl() + '/api/lancamentopagorecebido/' + lancamentoId + '/' + tipoTransacao,
                    headers: { 'Authorization': 'bearer ' + ApplicationScript.getToken() },
                    content: "application/json; charset=utf-8",
                    dataType: "json",
                    async: true,
    ...
    var getMasterViewListUrl = function () {
        ...
        //var lancamentoStatus = $("#cmbLancamentoStatus").val();
        ...
        var result = ApplicationScript.getAppWebApiUrl() + '/api/lancamentopagorecebido/masterViewList?' + jQuery.param(params);
    ...
    return {
        ...
        removeLancamento: function (lancamentoId, tipoTransacao, historico) {
            handleRemoveLancamento(lancamentoId, tipoTransacao, historico);
        }
    };
}();