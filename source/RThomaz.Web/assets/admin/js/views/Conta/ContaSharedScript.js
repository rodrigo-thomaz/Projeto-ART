var ContaSharedScript = function () {    

    var handleInit = function (controlId) {
               
        var tipoconta = $('#' + controlId).data().tipoconta;
        var url = '/api/conta/selectViewList';

        if (tipoconta != null) {
            url += '?tipoConta=' + tipoconta;
        }
        else{
            url += '?tipoConta=' + null;
        }
        ...
        $.ajax({
            ...
            for (var i = 0; i < data.length; i++) {
                if (tipoconta == null) {
                    ...
                    dataSource.push(convertToSelect2Model(data[i]));
                }
            } 
    ...
    var formatContaPoupancaResult = function (item) {
        ...
        itemTemplate += "			</div>";
        itemTemplate += "			<div>";
        itemTemplate += ' Ag: ' + item.model.dadoBancario.numeroAgencia + ' C/C: ' + item.model.dadoBancario.numeroConta + '</span><span class="badge badge-success pull-right">Conta poupança</span>';
        itemTemplate += "			</div>";
        itemTemplate += "		</div>";
        itemTemplate += "	</div>";
        itemTemplate += "</div>";
