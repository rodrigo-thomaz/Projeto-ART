var TipoTelefoneDetailScript = function () {
    ...
    var handleInit = function () {
        ...
        //Obtendo parametros
        tipoTelefoneId = getUrlParameter('id');
            ...
            $("#formDetail").validate({
                ...
                rules: {
                    txtNome: {
                        required: true,
                        maxlength: 255,
                        tipoTelefoneNomeExistente: true,
                    },                    
                },
        ...
        $("#btnCancelar").on("click", function () {
            window.location.href = "/TipoTelefone?tipoPessoa=" + tipoPessoa;
        });
    }

    var handleLoadForm = function () {

        if (tipoTelefoneId != null) {//Edit

            $.ajax({
                type: "GET",
                url: ApplicationScript.getAppWebApiUrl() + '/api/tipotelefone/' + tipoTelefoneId + '/' + tipoPessoa,
    ...
    var saveForm = function () {
        ...
        if (tipoTelefoneId == null) {
            ...
            $.ajax({
                type: "POST",
                url: ApplicationScript.getAppWebApiUrl() + '/api/tipotelefone',
                headers: { 'Authorization': 'bearer ' + ApplicationScript.getToken() },
                content: "application/json; charset=utf-8",
                dataType: "json",
                async: true,
...
jQuery.validator.addMethod("tipoTelefoneNomeExistente", function (value, element) {

    var tipoTelefoneId = getUrlParameter('id');
    var tipoPessoa = parseInt(getUrlParameter('tipo'));

    var url = ApplicationScript.getAppWebApiUrl();

    if (tipoTelefoneId == null) {
        url = url + "/api/tipotelefone/tipoPessoa/" + tipoPessoa + "/uniqueNome/" + value;
    }
    else {
        url = url + "/api/tipotelefone/" + tipoTelefoneId + "/tipoPessoa/" + tipoPessoa + "/uniqueNome/" + value;
    }
...
}, "J\u00e1 existe um tipo de telefone cadastrado com este nome.");
